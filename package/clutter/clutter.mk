...
CLUTTER_VERSION_MAJOR = 1.18
CLUTTER_VERSION_MINOR = 2
...
CLUTTER_CONF_OPT += --enable-egl-backend
CLUTTER_DEPENDENCIES += libegl
else
CLUTTER_CONF_OPT += --disable-egl-backend
endif

ifeq ($(BR2_PACKAGE_CLUTTER_BACKEND_WAYLAND),y)
CLUTTER_CONF_OPT += --enable-wayland-backend
CLUTTER_DEPENDENCIES += wayland gdk-pixbuf
else
CLUTTER_CONF_OPT += --disable-wayland-backend
endif

ifeq ($(BR2_PACKAGE_CLUTTER_BACKEND_X11),y)
CLUTTER_CONF_OPT += --enable-x11-backend
...
CLUTTER_CONF_OPT += --disable-x11-backend
endif

ifeq ($(BR2_PACKAGE_CLUTTER_INPUT_EVDEV),y)
CLUTTER_CONF_OPT += --enable-evdev-input
CLUTTER_DEPENDENCIES += \
	udev \
	libxkbcommon \
	libevdev \
